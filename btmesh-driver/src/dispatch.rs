...
impl Dispatcher {
    ...
    pub async fn dispatch(&self, message: AccessMessage<ProvisionedStack>) -> Result<(), DriverError>{
        ...
        if let Some(local_element_index) = local_element_index {
            ...
            self.device_sender.send( (Some(local_element_index as usize), opcode, Vec::from_slice(parameters)?, meta ) ).await;
        } else {
            self.foundation_sender.send( (None, opcode, Vec::from_slice(parameters)?, meta ) ).await;
            self.device_sender.send( (None, opcode, Vec::from_slice(parameters)?, meta ) ).await;
        }
