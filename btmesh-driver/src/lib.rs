        ...
        match (&pdu, &mut self.stack) {
            ...
            (PDU::Network(pdu), Stack::Provisioned(stack, sequence)) => {
                if let Some(result) = stack.process_inbound_network_pdu(pdu)? {
                    if let Some(block_ack) = &result.block_ack {
                        // send outbound block-ack
                        for network_pdu in stack.process_outbound_block_ack(sequence, block_ack)? {
                            self.network.transmit(&PDU::Network(network_pdu)).await;
                        }
                    }
