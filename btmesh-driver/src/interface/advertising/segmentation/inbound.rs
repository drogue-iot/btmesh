...
impl InboundSegments {
    ...
    pub(crate) fn receive(
        ...
        if self.segments[segment_index as usize].is_none() {
            let mut chunk = Vec::new();
            chunk
    ...
    pub(crate) fn fill<const N: usize>(&self, dst: &mut Vec<u8, N>) -> Result<(), DriverError> {
        for chunk in &self.segments {
            dst.extend_from_slice(chunk.as_ref().ok_or(DriverError::IncompleteTransaction)?)
                .map_err(|_| InsufficientBuffer)?;
        }
